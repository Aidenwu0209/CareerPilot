...
CareerPilot 当前以 Tailwind 硬编码 `pink-500 / pink-600 / pink-100 …` 作为事实上的品牌色，分散在 **61 个文件 + `globals.css`** 中（Header 下划线、CTA 按钮、AI 气泡、Loading、Tour、Landing 渐变等）。后果：
...
- 持久化：`localStorage.setItem('careerpilot-brand', 'jade'|'boss')`
