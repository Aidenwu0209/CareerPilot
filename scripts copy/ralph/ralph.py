...
def build_cmd(prompt: str) -> list[str]:
    ...
    if AGENT == "codex":
        ...
        return [executable, "exec", "--dangerously-bypass-approvals-and-sandbox", prompt]
    executable = shutil.which("claude.cmd" if os.name == "nt" else "claude") or "claude"
    return [executable, "--print", "--dangerously-skip-permissions", prompt]


def build_process_cmd(prompt: str) -> list[str]:
    """在 Unix 上使用 PTY；Windows 直接启动 agent，避免依赖 Unix script 命令。"""
    agent_cmd = build_cmd(prompt)
    script = shutil.which("script")
    if os.name != "nt" and script:
        return [script, "-q", "/dev/null"] + agent_cmd
    return agent_cmd

# 目录配置
SCRIPT_DIR = Path(__file__).parent.resolve()
PROJECT_ROOT = SCRIPT_DIR.parent.parent
WORKSPACE_ROOT = PROJECT_ROOT.parent.parent if PROJECT_ROOT.parent.name == ".worktrees" else PROJECT_ROOT
CLAUDE_INSTRUCTION_FILE = SCRIPT_DIR / "CLAUDE.md"
VALIDATOR_INSTRUCTION_FILE = SCRIPT_DIR / "VALIDATOR.md"
CANONICAL_RALPH_DIR = WORKSPACE_ROOT / "scripts" / "ralph"
PRD_FILE = (CANONICAL_RALPH_DIR if (CANONICAL_RALPH_DIR / "prd.json").exists() else SCRIPT_DIR) / "prd.json"
