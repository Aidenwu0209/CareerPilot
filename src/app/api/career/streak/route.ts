...
import { getStreak } from '@/lib/career/growth-service';
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(await getStreak(user.id, new Date(), timeZone));
  } catch (error) {
    return careerApiError(error, 'GET /api/career/streak');
  }
}
