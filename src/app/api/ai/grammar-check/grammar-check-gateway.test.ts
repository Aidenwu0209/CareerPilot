...
/**
 * US-042 tests: Grammar check route migrated to unified Gateway
 *
 * Validates:
 * - AC1: Resume ownership verified before gateway call
 * - AC2: Success returns issues/summary/score with operation tracked
...
async function seedProviderAndModel() {
  ...
  await db.insert(aiModels).values({
    id: 'grammar-check-default', providerId: 'p1',
...
describe('US-042: Grammar check via gateway', () => {
  beforeEach(async () => {
  ...
  it('returns grammar analysis on success', async () => {
    setUser('u1');
    mockGenerateText.mockResolvedValue({
      text: JSON.stringify({
        issues: [{ sectionId: 'summary', sectionTitle: 'Summary', type: 'grammar', original: 'I is', suggestion: 'I am', severity: 'high' }],
    ...
    const res = await POST(new Request('http://localhost/api/ai/grammar-check', {
      method: 'POST',
  ...
  it('returns error on insufficient credits', async () => {
    ...
    await db.update(aiModels).set({ fixedPrice: 99999 }).where(eq(aiModels.id, 'grammar-check-default'));
  ...
  it('ignores client-supplied x-api-key header', async () => {
    setUser('u1');
    mockGenerateText.mockResolvedValue({
      text: JSON.stringify({ issues: [], summary: 'All good.', score: 100 }),
