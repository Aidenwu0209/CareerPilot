...
需要 Python 3.11+ 和 Git。在 Ship Flow 源码仓库根目录运行：

```bash
/Users/wu/.local/share/mise/installs/python/3.12.13/bin/python3 scripts/install-codex-skill.py
```

上述是本机已验证的 Python 3.12。如果其他机器的 `python3 --version` 已是 3.11 或更高，可以直接使用 `python3`。
...
```bash
/Users/wu/.local/share/mise/installs/python/3.12.13/bin/python3 scripts/install-codex-skill.py --uninstall
```
