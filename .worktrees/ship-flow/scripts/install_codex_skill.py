...
if (
    ...
    fail("bundle metadata schema is invalid")
if hashlib.sha256(regular(Path(__file__))).hexdigest() != bundle["launcher_sha256"]:
...
"""
    return (
        template.replace("__PYTHON__", executable)
        .replace("__CODEX_HOME__", json.dumps(str(codex_home.resolve())))
